...
### Как пользоваться тестовым UI

1. В поле «Идентификатор пользователя» указать пользователя и нажать «Подключиться». До успешного SignalR-подключения кнопка «Создать ордер» отключена.
2. Заполнить «Клиентский ID», «Торговый символ», «Цена» и «Объём». Кнопка «Создать новый ID» генерирует уникальный idempotency key. Для проверки retry и conflict этот ID менять нельзя.
3. Нажать «Создать ордер». В блоке «События и ответы» отдельно отображаются HTTP-ответ и входящие SignalR-события.
4. В HTTP-записи проверить `httpКод`, `успешно` и `ответ`. Для ошибки `ответ` содержит стандартный `ProblemDetails`/`ValidationProblemDetails`.
5. В SignalR-записи проверить название события, `id`, `clientOrderId` и `isActive`. HTTP-ответ сам по себе не доказывает доставку WebSocket-события.
6. Перед независимым сценарием нажать «Очистить журнал» и «Создать новый ID», чтобы результаты предыдущей проверки не смешивались с текущими.
...
|---|---|---|---|
| Создание | Новый ID, `AAPL`, `225.50`, `10` | `201`, `успешно: true`, активный ордер | Ровно одно обновление с тем же `id`, `isActive: true` |
| Идемпотентный retry | Не менять ни ID, ни payload; нажать повторно | `200`, тот же `id` | Нового события нет |
| Конфликт ключа | После создания оставить ID, изменить price на `226` | `409 ProblemDetails` | Нового события нет, исходный ордер не изменён |
| Ошибка DTO | Новый ID, symbol `AA PL`, price `0`, volume `0` | `400 ValidationProblemDetails`, ошибки соответствующих полей | Событий нет |
| Превышение объёма | Новый ID, `AAPL`, `1`, `100001` | `422 ProblemDetails` | Событий нет |
| Превышение notional | Новый ID, `AAPL`, `1001`, `10000` | `422 ProblemDetails` | Событий нет |
| Автоотмена | Создать валидный ордер и ждать 15–17 секунд | Первый запрос остаётся `201` | Второе обновление того же `id`, `isActive: false` |
| Изоляция пользователей | Две вкладки `demo-user`, одна `another-user`; создать от `demo-user` | `201` | Событие только в двух вкладках `demo-user` |
| Нет пользователя | Очистить User ID и попытаться подключиться | REST недоступен, Hub handshake получает `401` | В журнале ошибка подключения; кнопка создания остаётся выключенной |
